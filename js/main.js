...
let getLocalDate = function (date, isSeconds, isISO) {
    switch (arguments.length) {
        ...
        case 2:
            return `${date.toLocaleDateString()}, ${isSeconds ? date.toLocaleTimeString().substring(0, 8) : date.toLocaleTimeString().substring(0, 5)}`;
        case 3:
            return `${date.toISOString().substring(0, 10)}, ${isSeconds ? date.toLocaleTimeString().substring(0, 8) : date.toLocaleTimeString().substring(0, 5)}`;
    }
};
let myDate = new Date();
...
console.log(getLocalDate(new Date(123456), true)); // 1970-01-01, 03:02:03 !!!возможно опечатка =>  01.01.1970, 03:02:03
...
let car2 = new Car(5000, 'FX50 AWD', 'Infiniti', 2020);
...
console.log(car2.info()); // Infiniti FX50 AWD, 2019cc, year 2019, new
car.used = 'used';
console.log(car2.info()); // Infiniti FX50 AWD, 2019cc, year 2019, new -- изменения не выполняются
console.log('-'.repeat(50));
